...
const propertySections = {
  ...
  projectDetails: {
    ...
    fields: {
      ...
      propertyHeight: { label: "Property Height", type: "number", placeholder: "Required" },
  ...
  unitDetails: {
    title: "Unit Details",
    fields: {
      unitNumber: { label: "Unit Number", type: "number", placeholder: "Optional" },
...
export function AddPropertyModal({ isOpen, onClose, propertyToEdit }: AddPropertyModalProps) {
  ...
  useEffect(() => {
    if (propertyToEdit) { 
      console.log("Initialized data:") // Add this line for debugging
      ...
      console.log("Initialized data:", initialData) // Add this line for debugging
    } else {
      resetForm()
    }
  }, [propertyToEdit])
    ...
    try {
      ...
      console.log(updatedData);
        response = await axios.put(
          `${process.env.NEXT_PUBLIC_CMS_SERVER}/property/updateSingleRecord`,
          updatedData,
        )
      } else {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="lg:max-w-5xl bg-zinc-950 text-white">
        <DialogHeader>
                  ...
                  field.type === "select" ? (
                    ...
                    <div key={fieldKey} className="space-y-2 mb-4">
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {(dataForm[fieldKey] || []).map((value: string, index: number) => (
                          <div
                            key={index}
                            className="flex items-center gap-1 px-3 py-1.5 bg-zinc-800/50 rounded-full text-sm"
                          >
                            {value}
                            <button
                    ...
                    <div key={fieldKey} className="col-span-2 space-y-4">
                      ...
                      <div className="grid grid-cols-3 md:grid-cols-6 gap-4">
                        {Array.from({ length: 6 }).map((_, index) => (
                          <div
                            key={index}
                            className="relative aspect-square rounded-lg border-2 border-dashed border-zinc-800 overflow-hidden"
                          >
                            {selectedImages[index] ? (
                              <>
                                ...
                                <button
                                  ...
                                  className="absolute top-2 right-2 p-1 rounded-full bg-zinc-900/80 hover:bg-zinc-800"
                                >
                                  <Minus className="h-4 w-4" />
                                </button>
                              </>
                            ) : index === selectedImages.length ? (
                              <label className="flex items-center justify-center w-full h-full cursor-pointer hover:bg-zinc-900/50">
                                <input
                                  type="file"
                                  accept="image/*"
                    ...
                    <div key={fieldKey} className="space-y-2 mb-4">
                      ...
                      <Input
                        ...
                        className={`bg-zinc-900 border-zinc-800 ${errors[fieldKey] ? "border-red-500" : ""}`}
                      />
                    </div>
                  ),
                )}
  ...
  return (
    <div className="space-y-2">
      ...
      <Select value={value} onValueChange={onChange}>
        <SelectTrigger id={fieldKey} className="bg-zinc-900 border-zinc-800">
