...
export function Sidebar() {
  ...
  return (
    <div className="pb-12 thin-scrollbar  w-64 border-r bg-[#913c3c] dark:bg-black text-white">
          ...
          {sidebarItems.map((item) => (
            <div key={item.title} className="space-y-1">
              <Button
                ...
                className={`w-[90%] flex justify-between gap-3 thin-scrollbar text-white hover:bg-[#9a2d5a] dark:hover:bg-white/20 hover:text-white transition-colors duration-200 ${
                  isMainItemActive(item.title) ? "bg-[#9a2d5a] dark:bg-white/20" : ""
                }`}
              >
              ...
              >
                {item.items.map((subItem) => (
                  <Link key={subItem} href={generateRoute(item.title, subItem)} passHref>
                    <Button
                      variant="ghost"
                      className={`w-full justify-start pl-12 text-sm text-gray-300 hover:bg-[#9a2d5a] dark:hover:bg-white/20 hover:text-white transition-colors duration-200 ${
                        isCurrentPage(item.title, subItem) ? "" : "" 
                      }`}
                    >
                      {subItem}
                    </Button>
