...
export default function StoresTable() {
  ...
  const fetchStores = async () => {
    setLoading(true)
    try {
      const response = await fetch("http://213.210.37.77:3016/stats/stores")
      if (!response.ok) {
  ...
  const handleStatusChange = async (storeId: string, suspend: boolean) => {
    try {
      const response = await fetch("http://213.210.37.77:3016/stats/suspend", {
        method: "POST",
        headers: {
     ...
     console.log('res',response)
      ...
      // Refetch stores after successful update
      await fetchStores()

      toast({
        title: "Status Updated",
        description: `Store status has been ${suspend ? "suspended" : "activated"}.`,
        variant: "default",
      })
    } catch (err) {
      console.error(err)
