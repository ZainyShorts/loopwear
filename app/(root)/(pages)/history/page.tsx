...
export default function OrderHistory() {
  ...
  const fetchOrderHistory = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://213.210.37.77:3016/order/history?userId=${userId}`)
              ...
              <TableBody>
                {orderData.orders.map((order) => (
                  <TableRow key={order._id}>
                    ...
                    <TableCell className="font-mono text-xs">{truncateId(order.storeId)}</TableCell>
                    <TableCell>
